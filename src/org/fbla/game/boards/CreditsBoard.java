...
//		g.setFont(new Font(Font.SANS_SERIF, Font.TYPE1_FONT, si));
		...
		drawOutlineString("Mary Ann Chambers", x("Mary Ann Chambers",g), st+(si*41)-i,g);
		
		drawOutlineString("Music & Audio:", x("Music & Audio:",g), st+(si*43)-i,g);
		drawOutlineString("Background music: HeatleyBros", x("Background music: HeatleyBros",g), st+(si*44)-i,g);
		drawOutlineString("Sound Effects: Bfxr.net", x("Sound Effects: Bfxr.net",g), st+(si*45)-i,g);
		
		drawOutlineString("Logo design: Tailorbrands.com", x("Logo design: Tailorbrands.com",g), st+(si*47)-i,g);
		...
		if(st+(si*(47+3))-i <= 90){
			...
			drawOutlineString("A big thanks to FBLA for giving us the opportunity to create this game!", x("A big thanks to FBLA for giving us the opportunity to create this game!",g), st+(si*(47+2))-i,g);
			g.drawImage(Texture.loadTexture("fbla-logo.png"), M_WIDTH/2 - 300, st+(si*(47+3))-i, 600, 465, this);
		}
