...
public class GameBoard extends Board implements ActionListener {
	...
	private void loadLevel3(boolean debug) {
		...
		for (int x = 0; x != 32; x++) {
			if (x >= 5 && x <= 25) {
				if ((x & 1) == 0)
					level3.add(new FallingFloor(x * 30, 15 * 30, Floor.GRASS));
			} else
				level3.add(new Floor(x * 30, 15 * 30, Floor.GRASS));
	...
	private void loadLevel5(boolean debug) {
		...
		for (int x = 0; x != 32; x++) {
			if (x <= 20) {
				if (x > 6)
					level5.add(new Floor(x * 30, (int) (3 * 30 + x * (x * 0.5)), Floor.GRASS, FloorBottom.GRASS));
				else
					level5.add(new Floor(x * 30, (int) (3 * 30 + x * (x * 0.5)), Floor.GRASS, FloorBottom.STONE));
			}
			level5.add(new Floor(x * 30, 16 * 30, Floor.GRASS));

		}
		level5.add(new Switch(63 * 15, 15 * 30, new Wall(7 * 30, 16 * 30, -(12 * 30), State.VERTICAL), level5,
				Rotation.LEFT, InteractionMethod.DISAPPEAR, SwitchType.RED));
	...
	private void loadLevel6(boolean debug) {
		...
		Switch s = new Switch(35 * 30, 35 * 30, new Sprite[] { new Wall(3 * 30 - 5, 0 * 30, 7 * 30, State.VERTICAL) },
				level6, Rotation.LEFT, InteractionMethod.DISAPPEAR, SwitchType.RED);

		if (!debug)
			s.interact();

		Switch s2 = new Switch(35 * 30, 35 * 30, new Sprite[] { new Wall(3 * 30, 7 * 30, 60, State.HORIZONTAL)

		}, level6, Rotation.LEFT, InteractionMethod.DISAPPEAR, SwitchType.RED);

		level6.add(new Switch(0, 6 * 30 - 7, new Sprite[] { s, s2 }, level6, Rotation.RIGHT, InteractionMethod.TRIGGER,
				SwitchType.RED));
	...
	private void loadLevel7(boolean debug) {
		...
		for (int x = 0; x != 16; x++) {
			if (x == 0)
				...
				level7.add(new Gold(x * 30 + 11, 10 * 30 - 11));
		}
	...
	private void loadLevel9(boolean debug) {
		...
		for (int x = 0; x != 32; x++) {
			...
			if (x < 20)
				...
				level9.add(new FallingFloor(x * 30, 17 * 30, Floor.GRAY_STONE));
		}
		...
		Switch s2 = new Switch(63 * 15, 31 * 15,
				new Sprite[] { new Ladder(19 * 30, 8 * 30), new Ladder(19 * 30, 9 * 30), new Ladder(19 * 30, 10 * 30),
						new Ladder(19 * 30, 11 * 30), new Ladder(19 * 30, 12 * 30), new Ladder(19 * 30, 13 * 30),
						new Ladder(19 * 30, 14 * 30), new Ladder(19 * 30, 15 * 30), new Ladder(19 * 30, 16 * 30),
	...
	public void loadLevel() {
		...
		if (Bridge.player.hasTool())
			Bridge.player.removeTool(Bridge.player.getTool());
		...
		if (Utils.getPlayerLevel() == 1) {
			Bridge.getPlayer().lives = 5;
			if (Utils.firsttime)loadHelp();
		}
	...
	private void drawInventory(Graphics g) {
		try {
			...
			g.drawImage(holder, B_WIDTH / 2 - (holder.getWidth(null)), B_HEIGHT / 2 - (holder.getHeight(null)),
					holder.getWidth(null) * 2, holder.getHeight(null) * 2, this);
			g.drawImage(img, B_WIDTH / 2 - (img.getWidth(null)), B_HEIGHT / 2 - (img.getHeight(null)),
					img.getWidth(null) * 2, img.getHeight(null) * 2, this);
	...
	public void drawLoading(Graphics g) {
		...
		g.drawString("LOADING...\nPlease wait..",
				B_WIDTH / 2 - g.getFontMetrics().stringWidth("LOADING...\nPlease wait..") / 2,
				B_HEIGHT / 2 - g.getFontMetrics().getHeight());
	...
	private void drawObjects(Graphics g) {
		...
		for (Sprite sprite : sprites) {
			...
			if (sprite instanceof Switch) {
				g.drawImage(sprite.getImage(), sprite.getX(), sprite.getY(), sprite.getWidth(), sprite.getHeight(),
						this);
			...
			g.drawImage(sprite.getImage(), sprite.getX(), sprite.getY(), (int) (sprite.getWidth()),
					(int) (sprite.getHeight()), this);
		...
		for (Moveable s : moveables) {
			...
			case FALLING_FLOOR:
				...
				g.drawImage(sprite.getImage(), sprite.getX(), sprite.getY(), (int) (sprite.getWidth() * extra),
						(int) (sprite.getHeight() * extra), this);
				break;
			case COMPETITOR:
				Competitor sk = (Competitor) s;
				g.drawImage(sprite.getImage(), (int) (sprite.getX() * extra), (int) (sprite.getY() * extra),
						(int) (Bridge.getPlayer().getWalkingWidth()), (int) (Bridge.getPlayer().getWalkingHeight()),
						this);
				((Competitor) s).drawHealthBar(g, sk.x - (50 / 2), sk.y - 20, 50, 5);
				break;
			case ARROW:

				// g.drawImage(Images.rotate(sprite.getImage(),
				// Math.asin(((Arrow) sprite).dx)/ Math.asin(((Arrow)
				// sprite).dy)), (int) (sprite.getX()*extra), (int)
				// (sprite.getY()*extra), (int) (16), (int) (4), this);
				try {
					if (((Arrow) sprite).getDirection().equals(Direction.LEFT))
						g.drawImage(sprite.getImage(), (int) ((sprite.x + 16) * extra), (int) (sprite.y * extra), -16,
								4, this);
					else
						g.drawImage(sprite.getImage(), (int) (sprite.getX() * extra), (int) (sprite.getY() * extra),
								(int) (16), (int) (4), this);
					...
					// sprite.x,
					// sprite.y,(int)p.getBounds().getWidth(),(int)p.getBounds().getHeight(),this);
			...
			default:
				g.drawImage(sprite.getImage(), (int) (sprite.getX() * extra), (int) (sprite.getY() * extra),
						(int) (sprite.getWidth()), (int) (sprite.getHeight()), this);
				break;
		...
		Bridge.getPlayer().drawHealthBar(g, (int) ((player.x - (100 / 2)) * extra), (int) ((player.y - 20) * extra),
				100, 5);
		...
		if (!Bridge.getPlayer().walking) {
			if (player.getFacingDirection().equals(Direction.LEFT))
				g.drawImage(player.getImage(), player.x + player.getRestingWidth(), player.y,
						-(player.getRestingWidth()), player.getRestingHeight(), this);
			...
			if (player.getFacingDirection().equals(Direction.RIGHT))
				g.drawImage(player.getImage(), player.x, player.y, player.getWalkingWidth(), player.getWalkingHeight(),
						this);
		}
		...
		g.drawImage(Texture.loadTexture("item-holder.png"),
				((B_WIDTH / 2 + B_WIDTH) / 2 + g.getFontMetrics().stringWidth("Tool:")), 25, this);
		if (Bridge.getPlayer().hasTool()) {
			Tool tool = ((Player) Bridge.getPlayer()).getTool();
			g.drawImage(tool.getImage(), ((B_WIDTH / 2 + B_WIDTH) / 2 + g.getFontMetrics().stringWidth("Tool:")) + 3,
					28, this);
		}
		if (debug && hitboxes)
		...
		drawOutlineString("Level: " + Bridge.getPlayer().level, (B_WIDTH / 2 + B_WIDTH) / 2, 60, g, Color.WHITE,
				Color.BLACK);
		...
		for (int life = 0; life != maxlives; life++) {
			if (life < Bridge.getPlayer().lives) {
				...
				g.drawImage(Texture.loadTexture("broken_heart.png"), life * 30, 1 * 15, 30, 30, this);
		}
				...
				if (info[0].contains("/n")) {
					...
					for (int i = 0; i != m.length; i++) {
						drawOutlineString(m[i], Integer.parseInt(info[1]),
								Integer.parseInt(info[2]) + (i * Integer.parseInt(info[5])), g, Color.WHITE,
								Color.BLACK);
					}
				} else
					drawOutlineString(info[0], Integer.parseInt(info[1]), Integer.parseInt(info[2]), g, Color.WHITE,
							Color.BLACK);
				if (time != 0)
				...
				if (info[0].contains("/n")) {
					...
					for (int i = 0; i != m.length; i++) {
						drawOutlineString(m[i], player.x + Integer.parseInt(info[1]),
								player.y + (Integer.parseInt(info[2]) + (i * Integer.parseInt(info[5]))), g,
								Color.WHITE, Color.BLACK);
					}
				} else
					drawOutlineString(info[0], player.x + Integer.parseInt(info[1]),
							player.y + Integer.parseInt(info[2]), g, Color.WHITE, Color.BLACK);
				if (time != 0)
	...
	public static void drawOutlinePolygon(Polygon p, Graphics g, Color inside, Color outsite) {
		...
		g.drawPolygon(p);
		g.setColor(outsite);
		g.drawPolygon(new int[] { p.xpoints[0] - 1, p.xpoints[1] + 1, p.xpoints[2] + 1, p.xpoints[3] - 1 },
				new int[] { p.ypoints[0] - 1, p.ypoints[1] - 1, p.ypoints[2] + 1, p.ypoints[3] + 1 }, 4);
		g.drawPolygon(new int[] { p.xpoints[0] + 1, p.xpoints[1] - 1, p.xpoints[2] - 1, p.xpoints[3] + 1 },
				new int[] { p.ypoints[0] + 1, p.ypoints[1] + 1, p.ypoints[2] - 1, p.ypoints[3] - 1 }, 4);
	}
	...
	private void drawGameOver(Graphics g) {
		...
		if (gameStatus.contains("gameover")) {
			...
			g.drawImage(Background.GAMEOVER.getImage(), 0, 0, Bridge.getGameBoardSize(0), Bridge.getGameBoardSize(1),
					this);
			g.setColor(Color.black);
	...
	private class TAdapter extends KeyAdapter {
		...
		public void keyPressed(KeyEvent e) {
			...
			// fs = true;
			// extra = ((double) Bridge.getGame().getWidth())/((double)
			// Bridge.getGameBoardSize(0));
			//
			// for(Sprite sprite : sprites){
			...
			// ((Sprite) sprite).height = (int) (((Sprite)
			// sprite).height*extra);
